...
export const PlayerBar = () => {
    ...
    const audioSrc = currentSong.fileUrl?.startsWith('http')
        ? currentSong.fileUrl
        : getSongStreamUrl(currentSong.id);
        ...
        <footer className="fixed bottom-0 left-0 right-0 h-24 bg-background/95 border-t border-primary/10 backdrop-blur-xl px-6 flex items-center justify-between z-50">
            ...
            <div className="flex items-center gap-4 w-1/3">
                <div className="size-14 rounded-lg bg-surface overflow-hidden shadow-lg border border-primary/20 shrink-0">
                    <img src={currentSong.artist?.avatar || currentSong.genre?.image || 'https://images.unsplash.com/photo-1614613535308-eb5fbd3d2c17'} alt={currentSong.title} className="w-full h-full object-cover" />
                </div>
                <div className="hidden sm:flex flex-col min-w-0">
                    <p className="text-sm font-bold text-white truncate">{currentSong.title}</p>
                    <p className="text-xs text-primary font-medium truncate">{currentSong.artist?.name || 'Unknown Artist'}</p>
                </div>
